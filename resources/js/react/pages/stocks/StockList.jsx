...
import { Boxes, Pencil, Trash2, Search, X } from "lucide-react";

const StockList = () => {
  const [search, setSearch] = useState("");
  const [editItem, setEditItem] = useState(null);
  const [stocks, setStocks] = useState([
    { id: 1, item: "روغن", location: "گدام کابل", quantity: 120 },
    { id: 2, item: "برنج", location: "گدام هرات", quantity: 85 },
    { id: 3, item: "چای", location: "گدام بلخ", quantity: 60 },
  ]);
        ...
        <table className="min-w-full bg-white dark:bg-gray-800 shadow-md rounded overflow-hidden text-sm">
          <thead>
            <tr className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
              ...
              <th className="py-2 px-4">محل گدام</th>
          ...
          <tbody>
            {filtered.map((s) => (
              <tr key={s.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="py-2 px-4">{s.item}</td>
