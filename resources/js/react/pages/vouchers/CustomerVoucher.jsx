...
import { WalletCards, Pencil, Trash2, Search, X } from "lucide-react";

const CustomerVoucherList = () => {
  const [search, setSearch] = useState("");
  const [editItem, setEditItem] = useState(null);
  const [vouchers, setVouchers] = useState([
    { id: 1, customer: "عبدالله", amount: 1500, date: "1403/03/01" },
    { id: 2, customer: "فاطمه", amount: 2000, date: "1403/03/05" },
  ]);
  ...
  const handleDelete = (id) => {
    if (window.confirm("آیا این واچر حذف شود؟")) {
  ...
  return (
    <div className="space-y-4 text-right rtl">
      <h2 className="text-2xl font-bold text-teal-700 dark:text-teal-400 flex items-center justify-end gap-2">
        <WalletCards size={24} /> لیست واچر مشتری
      </h2>
          ...
          <tbody>
            {filtered.map((v) => (
              <tr key={v.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="py-2 px-4">{v.customer}</td>
